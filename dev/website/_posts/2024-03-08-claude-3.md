---
title: Claude 3 beats GPT-4 on Dev's code editing benchmark
excerpt: Claude 3 Opus outperforms all of OpenAI's models on Dev's code editing benchmark, making it the best available model for pair programming with AI.
...
# Claude 3 beats GPT-4 on Dev's code editing benchmark

[![benchmark results](/assets/2024-03-07-claude-3.svg)](https://dev.chat/assets/2024-03-07-claude-3.svg)
...
2. If any tests failed, dev sends the LLM the failing test output and gives it a second try to complete the task.
...
Dev uses more efficient
[search/replace blocks](https://dev.chat/2023/07/02/benchmarks.html#diff)
with the original GPT-4
and
[unified diffs](https://dev.chat/2023/12/21/unified-diffs.html#unified-diff-editing-format)
...
- The Claude APIs seem somewhat unstable, returning HTTP 5xx errors of various sorts. Dev automatically recovers from these errors with exponential backoff retries, but it's a sign that Anthropic made be struggling under surging demand.
