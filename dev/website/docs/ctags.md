...
Dev no longer uses ctags to build a repo map.
Please see the newer article about
[using tree-sitter to build a better repo map](https://dev.chat/docs/repomap.html).
...
To address these issues, `dev` now
...
[chat transcript](https://dev.chat/examples/add-test.html)
...
To code with GPT-4 using the techniques discussed here:
  ...
  - Install [dev](https://dev.chat/docs/install.html).
  - Install universal ctags.
  - Run `dev` inside your repo, and it should say "Repo-map: universal-ctags using 1024 tokens".
...
This works pretty well, and is supported by `dev` -- you
...
The latest version of `dev` sends a **repo map** to GPT along with
...
Here's a
sample of the map of the dev repo, just showing the maps of
[main.py](https://github.com/Dev-AI/dev/blob/main/dev/main.py)
and
[io.py](https://github.com/Dev-AI/dev/blob/main/dev/io.py)
:
...
Mapping out the repo like this provides some benefits:
  ...
  - If it needs to see more code, GPT can use the map to figure out by itself which files it needs to look at. GPT will then ask to see these specific files, and `dev` will automatically add them to the chat context (with user approval).
...
Sending an appropriate subset of the repo map would help `dev` work
