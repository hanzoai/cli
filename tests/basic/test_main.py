...
from dev.main import check_gitignore, main, setup_git
from dev.utils import GitTemporaryDirectory, IgnorantTemporaryDirectory, make_repo
...
class TestMain(TestCase):
    def setUp(self):
        ...
        os.environ["DEV_CHECK_UPDATE"] = "false"
        os.environ["DEV_ANALYTICS"] = "false"
        self.original_cwd = os.getcwd()
        self.tempdir_obj = IgnorantTemporaryDirectory()
        self.tempdir = self.tempdir_obj.name
        os.chdir(self.tempdir)
        # Fake home directory prevents tests from using the real ~/.dev.conf.yml file:
        self.homedir_obj = IgnorantTemporaryDirectory()
        os.environ["HOME"] = self.homedir_obj.name
        self.input_patcher = patch("builtins.input", return_value=None)
        self.mock_input = self.input_patcher.start()
        self.webbrowser_patcher = patch("dev.io.webbrowser.open")
    ...
    @patch("dev.repo.GitRepo.get_commit_message", return_value="mock commit message")
    ...
    def test_main_with_git_config_yml(self):
        ...
        Path(".dev.conf.yml").write_text("auto-commits: false\n")
        with patch("dev.coders.Coder.create") as MockCoder:
            main(["--yes"], input=DummyInput(), output=DummyOutput())
            _, kwargs = MockCoder.call_args
            assert kwargs["auto_commits"] is False

        Path(".dev.conf.yml").write_text("auto-commits: true\n")
        with patch("dev.coders.Coder.create") as MockCoder:
    ...
    def test_main_with_empty_git_dir_new_subdir_file(self):
        ...
        # Because dev will try and `git add` a file that's already in the repo.
    ...
    def test_setup_git(self):
        ...
        self.assertEqual(".dev*", gitignore.read_text().splitlines()[0])
    ...
    def test_encodings_arg(self):
        ...
        with GitTemporaryDirectory():
            with patch("dev.coders.Coder.create") as MockCoder:  # noqa: F841
                with patch("dev.main.InputOutput") as MockSend:
    ...
    def test_main_exit_calls_version_check(self):
        with GitTemporaryDirectory():
            with (
                patch("dev.main.check_version") as mock_check_version,
                patch("dev.main.InputOutput") as mock_input_output,
            ):
                main(["--exit", "--check-update"], input=DummyInput(), output=DummyOutput())
                mock_check_version.assert_called_once()
                mock_input_output.assert_called_once()

    @patch("dev.main.InputOutput")
    @patch("dev.coders.base_coder.Coder.run")
    ...
    def test_dark_mode_sets_code_theme(self):
        ...
        with patch("dev.main.InputOutput") as MockInputOutput:
    ...
    def test_env_file_flag_sets_automatic_variable(self):
        env_file_path = self.create_env_file(".env.test", "DEV_DARK_MODE=True")
        with patch("dev.main.InputOutput") as MockInputOutput:
            MockInputOutput.return_value.get_input.return_value = None
    ...
    def test_default_env_file_sets_automatic_variable(self):
        self.create_env_file(".env", "DEV_DARK_MODE=True")
        with patch("dev.main.InputOutput") as MockInputOutput:
            MockInputOutput.return_value.get_input.return_value = None
            MockInputOutput.return_value.get_input.confirm_ask = True
    ...
    def test_false_vals_in_env_file(self):
        self.create_env_file(".env", "DEV_SHOW_DIFFS=off")
        with patch("dev.coders.Coder.create") as MockCoder:
    ...
    def test_true_vals_in_env_file(self):
        self.create_env_file(".env", "DEV_SHOW_DIFFS=on")
        with patch("dev.coders.Coder.create") as MockCoder:
            main(["--no-git", "--yes"], input=DummyInput(), output=DummyOutput())
            MockCoder.assert_called_once()
            _, kwargs = MockCoder.call_args
    ...
    def test_lint_option(self):
        with GitTemporaryDirectory() as git_dir:
            ...
            with patch("dev.linter.Linter.lint") as MockLinter:
    ...
    def test_verbose_mode_lists_env_vars(self):
        self.create_env_file(".env", "DEV_DARK_MODE=on")
        with patch("sys.stdout", new_callable=StringIO) as mock_stdout:
            ...
            relevant_output = "\n".join(
                ...
                if "DEV_DARK_MODE" in line or "dark_mode" in line
            )  # this bit just helps failing assertions to be easier to read
            self.assertIn("DEV_DARK_MODE", relevant_output)
            self.assertIn("dark_mode", relevant_output)
            self.assertRegex(relevant_output, r"DEV_DARK_MODE:\s+on")
    ...
    def test_yaml_config_file_loading(self):
        with GitTemporaryDirectory() as git_dir:
            ...
            # Create .dev.conf.yml files in different locations
            home_config = fake_home / ".dev.conf.yml"
            git_config = git_dir / ".dev.conf.yml"
            cwd_config = cwd / ".dev.conf.yml"
            named_config = git_dir / "named.dev.conf.yml"
            ...
            with (
                ...
                patch("dev.coders.Coder.create") as MockCoder,
            ):
    ...
    def test_map_tokens_option(self):
        with GitTemporaryDirectory():
            with patch("dev.coders.base_coder.RepoMap") as MockRepoMap:
    ...
    def test_model_metadata_file(self):
        ...
        with GitTemporaryDirectory():
            metadata_file = Path(".dev.model.metadata.json")
    ...
    def test_pytest_env_vars(self):
        ...
        self.assertEqual(os.environ.get("DEV_ANALYTICS"), "false")
    ...
    def test_invalid_edit_format(self):
        with GitTemporaryDirectory():
            with patch("dev.io.InputOutput.offer_url") as mock_offer_url:
                result = main(
                ...
                self.assertEqual(args[0], "https://dev.chat/docs/more/edit-formats.html")
