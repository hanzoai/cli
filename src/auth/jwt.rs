// jwt.rs — read a JWT's claims WITHOUT verifying its signature. Used only to
// display the local user's own identity (email/sub/owner/exp); it never grants
// trust, so base64url-decoding the middle segment is all that's needed.
...
/// intentionally NOT checked — the result is for display only.
