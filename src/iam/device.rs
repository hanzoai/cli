...
/// The device-flow client id for a brand: `<brand>-app` (the client IAM seeds
/// with the `device_code` grant enabled). The browser flow's `hanzo-cli` client
/// is a different, PKCE-only registration.
pub fn device_client_id(brand: &str) -> String {
    format!("{brand}-app")
}
...
mod tests {
    ...
    #[test]
    fn device_client_id_is_brand_app() {
        assert_eq!(device_client_id("hanzo"), "hanzo-app");
        assert_eq!(device_client_id("lux"), "lux-app");
        assert_eq!(device_client_id("zoo"), "zoo-app");
    }
